...
st.title('DASHBOARD DE VENDAS :shopping_trolley:') ## adicionou um emoji de carrinho
...
response = requests.get(url, params= query_string)          ## acessar o dados com get



# transformar a requisição pra json e o json para data frame
...
receita_mensal = dados.set_index('Data da Compra').groupby(pd.Grouper(freq='M'))['Preço'].sum().reset_index() ## tornou data da compra index e agrupou por mês, chamou a coluna preço, somou e resetou o index para voltar a ser coluna
...
fig_vendas_mensal = px.line(vendas_mensal,
                             ...
                             line_dash= 'Ano',
                             title= 'Vendas Mensal')
fig_vendas_mensal.update_layout(yaxis_title = 'Vendas Mensal')
...
coluna1, coluna2 = st.columns(2) ## variaveis receberam função da coluna
with aba1:
    coluna1, coluna2 = st.columns(2)
    with coluna1:
        st.metric('Receita', formata_numero(dados['Preço'].sum(), 'R$'))  ## função with identou a metrica
